...
class MultiInputLoon:
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.m = parsekw(kwargs.get('m'), 1.0)	# balloon mass

        self.x = parsekw(kwargs.get('xi'), 0.0)    # Initial x coordinate (m)  [default: 0 m]
        self.y = parsekw(kwargs.get('yi'), 0.0)    # Initial y coordinate (m)  [default: 0 m]
        self.z = parsekw(kwargs.get('zi'), 0.0)    # Initial z coordinate (m)  [default: 0 m]
        self.v_x = 0.0
        self.v_y = 0.0
        self.v_z = 0.0
        self.xi = self.x
        self.yi = self.y
        self.zi = self.z
        self.Fs = parsekw(kwargs.get('Fs'), 1.0)			# sampling frequency (Hz)   [default: Fs = 1 Hz]
    ...
    def update(self, *args, **kwargs):
        f_x = parsekw(kwargs.get('fx'), 0.0)
        f_y = parsekw(kwargs.get('fy'), 0.0)
        f_z = parsekw(kwargs.get('fz'), 0.0)
        a_x = parsekw(kwargs.get('ax'), 0.0)
        a_y = parsekw(kwargs.get('ay'), 0.0)
        a_z = parsekw(kwargs.get('az'), 0.0)
        v_x = parsekw(kwargs.get('vx'), 0.0)
        v_y = parsekw(kwargs.get('vy'), 0.0)
        v_z = parsekw(kwargs.get('vz'), 0.0)
    ...
    def __apply_accel__(self, *args, **kwargs):
        ax = parsekw(kwargs.get('ax'), 0.0)
        ay = parsekw(kwargs.get('ay'), 0.0)
        az = parsekw(kwargs.get('az'), 0.0)
    ...
    def __apply_pos__(self, *args, **kwargs):
        px = parsekw(kwargs.get('px'), 0.0)
        py = parsekw(kwargs.get('py'), 0.0)
        pz = parsekw(kwargs.get('pz'), 0.0)
